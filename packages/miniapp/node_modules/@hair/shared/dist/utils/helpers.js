 ...
 * 生成预约编号 YYYYMMDD-NNN
 */
export function generateAppointmentId(date, sequenceNum) {
    return `${formatDateCompact(date)}-${String(sequenceNum).padStart(3, '0')}`;
}
/**
 * 根据服务的 stages 和开始时间生成 timeline
 */
export function generateTimeline(stages, startTime) {
 ...
 */
export function getBusyRanges(timeline) {
    const ranges = [];
    let current = null;
 ...
 * 按固定步长生成时间段 slots
 */
export function generateTimeSlots(start, end, stepMinutes = 15) {
    const slots = [];
